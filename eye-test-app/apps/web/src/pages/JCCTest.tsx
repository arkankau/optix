...
import { useTestProgression, ProgressionEvents } from '../hooks/useTestProgression';
...
export default function JCCTest() {
  ...
  useTestProgression({
    currentStage: currentEye === 'OD' ? 'jcc_od' : 'jcc_os',
    onComplete: () => {
  ...
  const handleTranscript = async (text: string, _confidence: number) => {
    ...
    try {
      ...
      if (response.complete) {
        ...
        // Emit progression event instead of direct navigation
        // This allows the agent to trigger the flow
        if (currentEye === 'OD') {
          setPrompt('Great! Now let\'s test your left eye.');
          setTimeout(() => {
            ProgressionEvents.jccODComplete();
            // The progression hook will handle state changes
            // But we still need to reinit JCC for the new eye
            setTimeout(() => initJCC(), 500);
          }, 2000);
        } else {
          setPrompt('Excellent work! We\'re all done.');
          setTimeout(() => {
            ProgressionEvents.jccOSComplete();
          }, 2000);
        }
      } else {
        // Next comparison
