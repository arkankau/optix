...
class VisionEngine {
    constructor() {
        this.currentLambda = 0.008;
        this.currentModels = null;
        this.lfdInspired = false;
        this.runtimeAdapter = new runtime_adapter_1.RuntimeAdapter();
    }
    /**
     * Update PSF parameters and recompute models
     */
    updatePSF(params, lfd_inspired = false) {
        this.lfdInspired = lfd_inspired;
        const bypass = wavefront_engine_1.WavefrontEngine.isNoRx(params.sphere_D, params.cylinder_D);
    ...
    async processFrame(params) {
        ...
        const bypass = wavefront_engine_1.WavefrontEngine.isNoRx(params.psfParams.sphere_D, params.psfParams.cylinder_D);
     ...
     */
    processFrame2D(params, kernel, lambda) {
        const { buffer, width, height, contrast_boost } = params;
        const input = new Uint8Array(buffer);
        const result = wiener_engine_1.WienerEngine.processFrame(input, width, height, kernel, lambda, contrast_boost);
        return result.buffer;
    }
    /**
     * LFD-inspired pipeline
     */
    processFrameLFD(params, lut) {
        const { buffer, width, height } = params;
        const input = new Uint8Array(buffer);
        const result = lfd_lut_1.LFDLUTEngine.processFrame(input, width, height, lut);
        return result.buffer;
    }
    /**
     * Update lambda based on user feedback
     */
    updateLambda(feedback) {
        const step = 0.002;
