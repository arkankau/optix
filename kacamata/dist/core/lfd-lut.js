...
class LFDLUTEngine {
     ...
     */
    static buildLUT(wavefront, params, voxelSize = 4, raysPerVoxel = 6) {
        ...
        const astig_shear = this.computeAstigShear(wavefront.astig_magnitude, wavefront.astig_angle, params.distance_cm, ppi_val);
     ...
     */
    static computeDefocusOffset(defocus_D, distance_cm, ppi) {
     ...
     */
    static computeAstigShear(astig_magnitude, astig_angle, distance_cm, ppi) {
        if (astig_magnitude < 0.1)
            return { x: 0, y: 0 };
        const distance_m = distance_cm / 100;
     ...
     */
    static processFrame(input, width, height, lut) {
                    ...
                    for (let py = 0; py < voxel_size; py++) {
                        for (let px = 0; px < voxel_size; px++) {
                            ...
                            if (x0 >= 0 && x0 < width && y0 >= 0 && y0 < height) {
                                ...
                                const r = this.bilinearSample(input[idx00], input[idx01], input[idx10], input[idx11], fx, fy);
                                const g = this.bilinearSample(input[idx01 + 1], input[idx01 + 1], input[idx10 + 1], input[idx11 + 1], fx, fy);
                                const b = this.bilinearSample(input[idx00 + 2], input[idx01 + 2], input[idx10 + 2], input[idx11 + 2], fx, fy);
     ...
     */
    static bilinearSample(v00, v01, v10, v11, fx, fy) {
     ...
     */
    static microSharpen(input, width, height) {
        const output = new Uint8Array(input.length);
...
exports.LFDLUTEngine = LFDLUTEngine;
